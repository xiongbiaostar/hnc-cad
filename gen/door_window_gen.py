...
def pix2param(coord_full, SKETCH_PAD):
    ...
    return params
def plot(points, save_folder, name, mask_len):
    plt.figure()
    for i in range(len(mask_len)):
        room_points = points[i]
        for j in range(len(room_points)):
            next_index = (j + 1) % len(room_points)
            plt.plot([room_points[j][0], room_points[next_index][0]], [room_points[j][1], room_points[next_index][1]], color = 'green')

    for i in range(len(mask_len), len(points)):
        room_points = points[i]
        for j in range(len(room_points)):
            next_index = (j + 1) % len(room_points)
            plt.plot([room_points[j][0], room_points[next_index][0]], [room_points[j][1], room_points[next_index][1]], 'o-', color = 'red')
...
def sample(args):
    ...
    dataset = CADData(CAD_TRAIN_PATH, BOUNDARY_TRAIN_PATH, args.profile_code, args.loop_code, args.mode, is_training=False)
    ...
    count = 0
    for pixel_p, coord_p, sketch_mask_p, _, _, _, _, _, name, mask_len in dataloader:
        if count >= 100: break
        ...
        plot(param, result_folder, name, mask_len)
