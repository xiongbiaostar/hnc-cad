...
def coord2param(coord_full, type_full, SKETCH_PAD):
    ...
    params = []
    param = []
    param.append(np.array([type_full[0], type_full[0]]))
    for i in range(0, len(coord_full)):
        if np.array_equal(coord_full[i], np.array([-2, -2])):
            break
        if np.array_equal(coord_full[i], np.array([-1, -1])):
            params.append(param)
            param = []
            param.append(np.array([type_full[i+1], type_full[i+1]]))
            continue
        param.append(coord_full[i])
    return params
...
def sample(args):
    ...
    count = 0
    for pixel_p, coord_p, type_p, sketch_mask_p, _, _, _, _, _, _, name, boundaries in dataloader:
        ...
        sketch_latent = sketch_enc(pixel_p, coord_p, type_p, sketch_mask_p)
        ...
        xy_samples, type_samples, _code_, _code_mask_, _latent_z_, _latent_mask_ = sketch_dec.sample(total_code, total_code_mask,pixel_p, coord_p, type_p, sketch_latent, sketch_mask_p,top_k=1, top_p=0)
        if len(xy_samples) >= 2:
            param_pred_sel1 = coord2param(xy_samples[0], type_samples[0], SKETCH_PAD)
            param_pred_sel2 = coord2param(xy_samples[1], type_samples[1], SKETCH_PAD)
        else:
            param_pred_sel1 = param_pred_sel2 = coord2param(xy_samples[0], type_samples[0], SKETCH_PAD)
        GT_boundaries = [tensor.squeeze(0).numpy().tolist() for tensor in boundaries]
        coord_ori = coord_p.cpu().numpy()[0]
        type_ori = type_p.cpu().numpy()[0]
        param_ori = coord2param(coord_ori, type_ori, SKETCH_PAD)
