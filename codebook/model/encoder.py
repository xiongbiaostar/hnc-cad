...
class RPLANEncoder(nn.Module):

  def __init__(self):
    """
    Initializes Solid Model.
    """
    super(RPLANEncoder, self).__init__()
    ...
    self.pos_embed = PositionalEncoding(max_len=MAX_RPLAN, d_model=self.embed_dim)
    ...
    decay = 0.99
    self.code_dim = 256
    self.codebook = VectorQuantizerEMA(RPLAN_CODEBOOK_DIM, self.code_dim, commitment_cost, decay)
