...
class RPLANDecoder(nn.Module):
  ...
  def __init__(self):
    ...
    Initializes model.
    """
    super(RPLANDecoder, self).__init__()
    self.embed_dim = DECODER_CONFIG['embed_dim']
    self.param_embed = Embedder(2**BIT, 32)
    self.param_fc = nn.Sequential(
        nn.Linear(32*RPLAN_PARAM_SEQ, self.embed_dim),
        nn.BatchNorm1d(self.embed_dim),
        nn.LeakyReLU(),
    )

    self.pos_embed = PositionalEncoding(max_len=MAX_RPLAN+1, d_model=self.embed_dim)
    ...
    self.param_logit1 = nn.Linear(self.embed_dim, 32*RPLAN_PARAM_SEQ)
  ...
  def forward(self, param, seq_mask, ignore_mask, latent_code):
    ...
    param_logits2 = self.param_logit2(param_logits1.view(param_logits1.shape[0], param_logits1.shape[1], RPLAN_PARAM_SEQ, -1))
